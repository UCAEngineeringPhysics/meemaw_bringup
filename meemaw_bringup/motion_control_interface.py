...
class MotionControlInterface(Node):
    def __init__(self):
        super().__init__("motion_control_interface")
        # Create serial communication to Pico
        self.messenger = serial.Serial("/dev/ttyACM0", 115200, timeout=0.01)
        self.pico_listener_timer = self.create_timer(0.015, self.get_meas_vel)
        # Create target velocity subscriber
        self.ref_vel_subr = self.create_subscription(
            topic="cmd_vel",
            msg_type=Twist,
        ...
        # variables
        self.meas_lin_vel = 0.0  # in base_link frame
        self.meas_ang_vel = 0.0  # and they are real vels
        ...
        self.get_logger().info("Meemaw's motion control is ready.")

    def get_meas_vel(self):
        if self.messenger.inWaiting() > 0:
            vels = (
                self.messenger.readline().decode("utf-8", "ignore").strip().split(",")
            )  # actual linear and angular vel
            if len(vels) == 2:
                try:
                    self.meas_lin_vel = float(vels[0])
                    self.meas_ang_vel = float(vels[1])
                except ValueError:
    ...
    def set_targ_vel(self, msg):
        ...
        self.messenger.write(f"{targ_lin:.4f},{targ_ang:.4f}\n".encode("utf-8"))
        self.get_logger().debug(
...
def main(args=None):
    ...
    motion_control_interface = MotionControlInterface()
    rclpy.spin(motion_control_interface)
    motion_control_interface.destroy_node()
    rclpy.shutdown()
